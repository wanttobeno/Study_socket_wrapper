...
public:
...
#endif
	inline static sockaddr_in GetAddress(const char* ipAddress, int port) {
  ...
  inline bool Connect(const char *url, int port) {
	...
	int nRet = connect(sockfd_, reinterpret_cast<const sockaddr*>(&server_addr), sizeof(sockaddr_in));
    if (nRet == 0) {
      return true;
    }
    return false;
  ...
  inline bool ConnectUrl(const char* url, int port)
  {
